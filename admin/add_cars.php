					...
					<div class="box">
						...
						<form action="" method="post" enctype="multipart/form-data">
							<div class="form">
								...
								<p>
									<span class="req">Vehicle Owner Mobile</span>
									<label>Mobile Number<span>(Required Field)</span></label>
									<input type="text" class="field size1" name="mobile" required />
								</p>
							</div>
							<div class="buttons">
						...
						if (isset ($_POST['send'])) {
							...
							if (move_uploaded_file($_FILES['image']['tmp_name'], $target_path)) {
								...
								$mobile = $_POST['mobile'];
								$qr = "INSERT INTO cars (image, car_name,car_type,hire_cost,capacity,mobile,status) 
													VALUES ('$image','$car_name','$car_type','$hire_cost','$capacity',
													'$mobile',
													'Available')";
								$res = $conn->query($qr);
								if ($res === TRUE) {
